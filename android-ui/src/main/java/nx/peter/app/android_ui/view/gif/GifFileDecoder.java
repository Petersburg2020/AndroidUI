package nx.peter.app.android_ui.view.gif;
...
public class GifFileDecoder {
  ...
  public GifFileDecoder(InputStream in) {
    file = null;
    tIn = in;
  }
  ...
  public void start() throws IOException {
    ...
    if (!id.startsWith("GIF")) throw EX_IFF;
    ...
    for (int i = 0; i < ltableTemplate.length; i++) ltableTemplate[i] = 1;
  ...
  private void reopenStream() throws IOException {
    if (in != null) in.close();
    if (file != null) in = new BufferedInputStream(new FileInputStream(file), 32768);
    else if (tIn != null) in = tIn;
  }
  ...
  public int[] readFrame() throws IOException {
    ...
    if (frameIndex == 0) throw new IOException("No GIF frames in file");

    loopIndex++;
    if (loopCount != 0 && loopIndex >= loopCount) return null;
  ...
  private void readGraphicControlExt() throws IOException {
    ...
    if (dispose != DISPOSE_BACKGROUND && dispose != DISPOSE_PREVIOUS) dispose = DISPOSE_NONE;
    transparency = (packed & 1) != 0;
    delay = readShort() * 10; // delay in milliseconds
    if (delay < MIN_DELAY) delay = ENFORCED_DELAY;
    ...
    while (pi < npix) {
      ...
      if (!fl) {
        ...
        if (old_code > clear) {
          ...
          for (int i = 0; i < len; i++) _pixels[pi++] = _pixels[p + i];
        } else _pixels[pi++] = _act[old_code];
      ...
      if (fl) {
        if (code == clear) {
          ...
          if (code > clear) {
            ...
            for (int i = 0; i < len; i++) _pixels[pi++] = _pixels[p + i];
          } else _pixels[pi++] = _act[code];
        }
      }
      old_code = code;
    }
    for (int i = pi; i < npix; i++) _pixels[i] = 0; // clear missing pixels
  }
  ...
  private int[] drawImage() {
    ...
    if (dispose == DISPOSE_NONE) {
      ...
      if (tempImage == null) tempImage = new int[width * height];
    ...
    if (!interlace
        ...
        && iy == 0
        && iw == width
        && ih == height) drawImageFaster(dest);
    else drawImageRegular(dest);
    ...
    for (int i = 0; i < ih; i++) {
      ...
      if (line < height) {
        ...
        while (dx < dlim) {
          ...
          if (dispose == DISPOSE_BACKGROUND) baseImage[dx] = 0;
          dx++;
        }
      }
    }
  ...
  private int readBlock() throws IOException {
    ...
    if (size == -1) throw EX_EOF;
    int n = 0;
    while (n < size) {
      final int count = in.read(block, n, size - n);
      if (count == -1) throw EX_EOF;
      n += count;
    }
    return size;
  }
  ...
  public static class Frame {
    ...
    public final int width, height, position;

    public Frame(int[] pixels, int width, int height, int position) {
      this.pixels = pixels;
      this.width = width;
      this.height = height;
